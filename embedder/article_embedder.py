...
class ArticleEmbedder:
    ...
    def embed_article(self, article: str | list[str], return_numpy: bool = True) -> np.ndarray:
        """
        ...
        """
        logger.info(f"⏳ Đang encode bài báo {'(nhiều)' if isinstance(article, list) else '(1 bài)'}")
        embeddings = self.embedder.encode(article, return_numpy=return_numpy)
        logger.info("✅ Đã encode xong bài báo")
        return embeddings
